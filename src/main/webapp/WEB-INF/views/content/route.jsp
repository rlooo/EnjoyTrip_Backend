    ...
    <script>
      ...
      placeSearchBtn.addEventListener("click", async function () {
        ...
        if (inputPlace.value) {
          let places = await fetch(
            "/content/searchPlace/" + inputPlace.value,
          ...
          if (planMarker.length > 0) {
            for (let i = 0; i < planMarker.length; i++) {
              markerList.push(
                Object.assign(new kakao.maps.Marker(), planMarker[i]),
              );
              infoList.push(
                Object.assign(new kakao.maps.InfoWindow(), planInfoWindow[i]),
              );
            }
          }
          places = places.filter(
            (item) => !planPlace.some((c) => c.contentId == item.contentId),
          );
          ...
          for (let i = 0; i < places.length; i++) {
            ...
            bounds.extend(
              new kakao.maps.LatLng(
                Number(places[i].mapY),
                Number(places[i].mapX),
              ),
            );
          }
      ...
      function displayMarker(place) {
        ...
        const marker = new kakao.maps.Marker({
          ...
          image: new kakao.maps.MarkerImage(
            "${root}/assets/img/marker/" + locImage[place.contentType] + ".png",
        ...
        kakao.maps.event.addListener(
          marker,
          "click",
          makeClickListener(marker, infoWindow, place),
        );
      }
      ...
      function addPlanPlace(marker, place, uuid) {
        ...
        for (let i = 0; i < markerList.length; i++) {
          if (
            markerList[i].getPosition().getLat().toPrecision(14) ==
              place.mapY &&
            markerList[i].getPosition().getLng().toPrecision(14) == place.mapX
          ) {
            // markerList.splice(i, 1);
            planInfoWindow.push(
              Object.assign(new kakao.maps.InfoWindow(), infoList[i]),
            );
            // infoList.splice(i, 1);
          }
        }
      ...
      function addMarkerLinePath(marker) {
        linePath.push(
          Object.assign(new kakao.maps.LatLng(), marker.getPosition()),
        );
      ...
      function addPlanBtn(marker, place) {
        ...
        planInfoDiv.setAttribute(
          "class",
          "text-start border py-1 rounded-2 my-1 placeList-item draggable",
        );
        planInfoDiv.draggable = true;
        planInfoDiv.addEventListener("dragstart", () =>
          handlerDragStart(planInfoDiv),
        );
        planInfoDiv.addEventListener("dragend", () =>
          handlerDragEnd(planInfoDiv),
        );
        ...
        planInfoTitleDiv.style =
          "background:#eee; margin-top:-0.21rem; border-radius:0 0.2rem 0 0; font-size:0.8rem";
        ...
        planInfoTextArea.setAttribute(
          "class",
          "form-control my-1 plan-place-memo",
        );
        ...
        planInfoBtnDiv.setAttribute(
          "class",
          "row d-flex justify-content-between align-items-center px-1",
        );
        ...
        planInfoLink.href =
          "http://data.visitkorea.or.kr/resource/" + place.contentId;
        ...
        planInfoDelBtn.setAttribute(
          "class",
          "col-auto btn btn-sm btn-danger me-2",
        );
      ...
      function makeInfo(marker, place) {
        ...
        infoContentRightDiv.setAttribute(
          "class",
          "col-md-7 align-self-center flex-wrap mb-1 ps-1",
        );
        ...
        infoContentRightBtnDiv.setAttribute(
          "class",
        ...
        infoContentRightLinkA.setAttribute(
          "href",
          "http://data.visitkorea.or.kr/resource/" + contentId,
        );
        ...
        infoContentRightBtn.setAttribute(
          "class",
          "col-auto btn btn-sm btn-primary me-3",
        );
        infoContentRightBtn.innerText = "추가";
        infoContentRightBtn.addEventListener("click", () =>
          addPlanBtn(marker, place),
        );
      ...
      document.querySelector("#save-plan-btn").addEventListener("click", () => {
        ...
        if (contents.length == 0) {
          ...
          let memoList = Array.from(
            document.querySelectorAll(".plan-place-memo"),
          ).map((el) => el.value);
          let contentList = [];
          ...
          fetch(`/content/plan/write`, config).then((response) =>
      ...
      function getDragAfterEl(box, y) {
        const draggingItems = [
          ...placeList.querySelectorAll(".draggable:not(.dragging)"),
        ];
        return draggingItems.reduce(
          (closest, child) => {
